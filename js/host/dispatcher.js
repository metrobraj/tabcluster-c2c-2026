...
class TCDispatcher {
  ...
  addWorker(peerId) {
    ...
    if (this.currentJob && this.currentFnSource) {
      this.transport.send(peerId, tcMakeMessage(TC_MSG.JOB_INIT, {
        pluginId: this.currentJob, fnSource: this.currentFnSource, resultType: this.resultType
      }));
    }
    this._emitTelemetry();
  ...
  //   fnSource:       JS source text of `function(task) { ...; return result; }`
  //   resultType:     overrides the plugin's defaultResultType if given
  startJob({ pluginId, splitterParams, fnSource, resultType }) {
    ...
    for (const peerId of this.workers) {
      this.transport.send(peerId, tcMakeMessage(TC_MSG.JOB_INIT, { pluginId, fnSource, resultType: this.resultType }));
    }
